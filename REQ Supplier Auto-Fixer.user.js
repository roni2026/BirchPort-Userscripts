...
(function () {
  ...
  function buildUI() {
    ...
    // Auto-fill "Max lines this run" to the number of line items actually
    // on this REQ page, instead of the hardcoded default of 25 - so a full
    // run isn't silently capped short just because the REQ has more lines
    // than the old default. A resumed run's saved budget (below) still
    // takes precedence over this, and the user can still edit it by hand.
    const initialItemCount = scanReqLines().length;
    if (initialItemCount > 0) {
      capInput.value = String(initialItemCount);
    }

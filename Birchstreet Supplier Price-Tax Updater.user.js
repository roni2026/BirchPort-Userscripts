...
// @version      2.0
// @description  Floating, draggable, minimizable panel with Single and Bulk modes. Single: fill Item SKU + Supplier + Trx Currency + Unit Price + optional Tax Code 1/2. Bulk: paste a tab-separated table (SKU / Desc / Supplier / UOM / Price / Currency) and it processes every row matched by SKU.
// @author       roni2026
...
(function () {
  ...
  // v2.0 changes:
  //
  //  1. CURRENCY-CODE-TOLERANT COMPARISON: v1.9 only stripped symbols
  //     ($, £, €, ₹) before comparing what got typed vs. what Birchstreet
  //     shows. Birchstreet also prefixes fields with letter currency
  //     codes ("MVR25.0000" for a typed "25"), and pads with trailing
  //     zeros. normForCompare() now also strips known 3-letter currency
  //     codes, and the comparison (valuesMatch()) falls back to a
  //     NUMERIC comparison whenever both sides parse as numbers, so
  //     "25" and "MVR25.0000" are treated as equal. Non-numeric fields
  //     (supplier names, tax codes) still compare as normalized text.
  //
  //  2. ORDER: Trx Currency is now filled BEFORE Unit Price (previously
  //     price was filled first), matching how you actually work through
  //     the dialog. Tax Code 1/2 stay last and are skipped whenever left
  //     blank.
  //
  //  3. AUTO-DISMISS SAVE POPUP: right before clicking the page-level
  //     Save button, the script patches window.alert/window.confirm on
  //     every reachable frame so a Birchstreet "saved" popup that shows
  //     up a few seconds later is logged and auto-accepted instead of
  //     blocking the run. (If Birchstreet ever throws a genuine
  //     browser-native dialog instead of a JS alert/confirm, this can't
  //     reach it - you'd still have to dismiss that one by hand.)
  //
  //  4. BULK MODE: a Mode dropdown switches the panel between Single
  //     (the original one-item form) and Bulk. Bulk gives you a text box
  //     to paste rows copied straight out of a sheet like:
  //         Item SKU <tab> Product Desc. <tab> Supplier <tab> UOM <tab> Price <tab> Currency
  //     Matching is done on SKU only (column 1), never on the product
  //     description, since portal naming can differ from your sheet.
  //     For each row it: searches the SKU, opens Supplier Items, opens
  //     the matching supplier's row, sets Currency then Price (Tax 1/2
  //     always left blank in bulk), clicks OK, clicks Save, waits ~5s
  //     for the save popup to clear, then moves to the next row. A
  //     failed row is logged and skipped rather than stopping the whole
  //     batch; a summary is printed at the end.
  // -----------------------------------------------------------------------
  ...
  function initPanel() {
    ...
    const KNOWN_CURRENCY_CODES =
      ['mvr', 'usd', 'eur', 'gbp', 'inr', 'aed', 'sar', 'sgd', 'aud', 'jpy'];
    ...
    // Strips currency symbols, known 3-letter currency codes, and
    // thousands separators so a value like "MVR1,950.00" normalizes the
    // same as "1950.00" for comparison purposes. Only ever used to
    // decide "did the field actually take the value" - never used to
    // overwrite what's displayed on screen.
    function normForCompare(s) {
      ...
      for (
        const code of KNOWN_CURRENCY_CODES
      ) {
    ...
    // Compares two field values tolerant of Birchstreet's own
    // auto-formatting. If both sides reduce to plain numbers after
    // stripping currency codes/symbols, compares numerically (so "25"
    // matches "MVR25.0000"). Otherwise falls back to normalized text
    // comparison (supplier names, tax codes, SKUs, etc.).
    function valuesMatch(actual, expected) {

      const a = normForCompare(actual);
      const e = normForCompare(expected);
    ...
    // Patches window.alert / window.confirm on every reachable frame so
    // a Birchstreet "record saved" popup fired a few seconds after Save
    // gets logged and auto-accepted instead of blocking the run. Safe
    // to call repeatedly - re-patches idempotently per frame.
    function installAutoAccept(rootWin) {

      const frames =
        getReachableFrames(rootWin);
      ...
      for (
        ...
        try {
          ...
          win.confirm = function (msg) {
            ...
            log(
              `Auto-accepted popup: "${msg}"`
            );

            return true;
          };

          win.__bskAutoAccept = true;
          win.__bskOriginalAlert = originalAlert;
          win.__bskOriginalConfirm = originalConfirm;

        } catch (e) {
          // Cross-origin or otherwise unreachable - skip.
        }
      }
    }
    ...
    // FILL + OK + SAVE
    //
    // Order: Trx Currency, THEN Unit Price, then Tax 1, then Tax 2
    // (blank tax fields are skipped by setDirectField above).
    // ===================================================================
    ...
    async function fillFieldsAndSave(
      ...
      // ---------------------------------------------------------------
      // Tax 1 / Tax 2 (skipped automatically when blank)
      // ---------------------------------------------------------------
      ...
      // Patch alert/confirm on every reachable frame right before Save,
      // so the "saved" popup that shows up a few seconds later gets
      // logged and auto-accepted instead of blocking the run.
      installAutoAccept(entryWin);
      installAutoAccept(window.top);
      ...
      log(
        `Page-level Save clicked. Waiting ~${Math.round(POST_SAVE_DELAY_MS / 1000)}s for the save popup to clear...`
      );
    ...
    // Parses pasted, tab-separated rows shaped like:
    //   Item SKU <tab> Product Desc. <tab> Supplier <tab> UOM <tab> Price <tab> Currency
    // Matching is always by SKU (column 1) - the description is ignored
    // since portal naming can differ from the sheet.
    function parseBulkRows(text) {
      ...
      for (
        ...
        if (cols.length < 5) {
          ...
          log(
            `Bulk parse: skipping unrecognized row (need at least 5 columns): "${line}"`
          );
        ...
        if (!sku || !supplier || !price) {

          log(
            `Bulk parse: skipping row missing SKU/Supplier/Price: "${line}"`
          );
      ...
      <div id="bskBody" style="padding: 14px;">
        ...
        <div style="margin-bottom:10px;">
          ...
          >
            ...
            <option value="bulk">Bulk (paste multiple rows)</option>
          </select>
        </div>

        <div id="bskSingleFields">
          ...
          <div style="margin-bottom:7px;">
            <label>Tax Code 1 (optional)</label>
            <input
              id="bskTax1"
              type="text"
              value=""
          ...
          <div style="margin-bottom:10px;">
            <label>Tax Code 2 (optional)</label>
            <input
              id="bskTax2"
              type="text"
              value=""
        ...
        <div id="bskBulkFields" style="display:none;">

          <div style="margin-bottom:7px;">
            <label>
              Paste rows (tab-separated: SKU, Desc, Supplier, UOM, Price, Currency).
              Matching is by SKU only - description is ignored.
            </label>
            <textarea
              id="bskBulkText"
              placeholder="14156&#9;JUICE - GRANINI APPLE JUICE 250 ML&#9;EURO MARKETING PRIVATE LIMITED&#9;CS&#9;10&#9;MVR"
              style="
                width:100%;
                box-sizing:border-box;
                padding:7px;
    ...
    const modeSelect =
      document.getElementById('bskMode');

    const singleFieldsEl =
      document.getElementById('bskSingleFields');

    const bulkFieldsEl =
      document.getElementById('bskBulkFields');

    function applyMode(mode) {

      singleFieldsEl.style.display =
        mode === 'bulk' ? 'none' : 'block';

      bulkFieldsEl.style.display =
        mode === 'bulk' ? 'block' : 'none';
    ...
    // ===================================================================
    // CORE PIPELINE (shared by Single and Bulk)
    // ===================================================================
    ...
    runBtn.addEventListener(
      ...
      async () => {
        ...
        try {
          ...
          GM_setValue('bskSupplier', supplier);
          GM_setValue('bskCurrency', currency);
          GM_setValue('bskTax1', tax1);
          GM_setValue('bskTax2', tax2);
        ...
        try {
          ...
          log(
            `Starting single update: SKU=${sku}, Supplier="${supplier}", Currency="${currency || '(unchanged)'}", Price=${price}, Tax1="${tax1 || '(skip)'}", Tax2="${tax2 || '(skip)'}"`
          );
    ...
    runBulkBtn.addEventListener(
      'click',
      async () => {

        const bulkTextEl =
          document.getElementById('bskBulkText');
        ...
        log(
          `Starting bulk update: ${rows.length} row(s) parsed.`
        );
        ...
        for (
          ...
          setStatus(
            `Processing ${i + 1}/${rows.length}: SKU ${row.sku}...`
          );
          ...
          log(
            `Row ${i + 1}/${rows.length}: SKU=${row.sku}, Supplier="${row.supplier}", Currency="${row.currency || '(unchanged)'}", Price=${row.price}`
          );
          ...
          try {
            ...
            console.error(
              '[BSK Price Updater] Bulk row failed:',
              row,
              err
            );
          }
        ...
        setStatus(
          `Bulk finished: ${succeeded} succeeded, ${failed} failed.`
        );
        ...
        log(
          `Bulk finished: ${succeeded} succeeded, ${failed} failed out of ${rows.length}.`
        );
    ...
    log(
      'Single mode: enter SKU + Supplier and click Run Update. Bulk mode: paste rows and click Run Bulk.'
    );
  }

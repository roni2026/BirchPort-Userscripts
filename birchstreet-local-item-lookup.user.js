...
// @author       Rick
// @match        https://app02.birchstreetsystems.com/*
// @grant        GM_setValue
// @grant        GM_getValue
...
(function () {
    ...
    const findLocalItemMaintenanceLeaf = () =>
        document.querySelector('[title="Local Item Maintenance"]') || document.getElementById('HN7');

    const findSearchFieldSelect = () => document.getElementById('SearchField');
    const findSearchValueInput = () => document.getElementById('SearchValue');
    const findSearchButton = () => document.getElementById('searchButton');
    const findFirstResultRow = () => document.querySelector('tr[id^="tb"]');

    const findSupplierItemsTab = () => {
        const byId = document.getElementById('tab3');
        if (byId && byId.textContent.trim() === 'Supplier Items') return byId;
        const candidates = Array.from(document.querySelectorAll('td[onclick*="setTab"]'));
        return candidates.find((td) => td.textContent.trim() === 'Supplier Items') || null;
    };

    const findSupplierGridTable = () => document.getElementById('TABLE_GRID');
    ...
    async function step3_openFirstResult(partNumber) {
        ...
        const row = await waitFor(findFirstResultRow, { timeout: 15000 });
        row.click();
    }
